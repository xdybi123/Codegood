...
// Images are stored in RAM as System.Drawing.Bitmap.
//
...
namespace DigitalMicrograph
{
    ...
    {
        /// <summary>Bitmap stored entirely in RAM (greyscale normalised, or RGBA for colour images).</summary>
    ...
    public class DmReader
    {
        ...
        private void ExtractImages(TagNode root)
        {
            ...
            foreach (TagNode entry in imageList.Children)
            {
                ...
                // If there are more than 2 dims (stack/spectrum image) take first 2D slice.
        ...
        private Bitmap BuildBitmap(long dataOffset, int width, int height, int dataType)
        {
            ...
            // RGBA images (codes 8 and 23): stored as B,G,R,A bytes per pixel.
            if (dataType == 8 || dataType == 23)
                return BuildRgbaBitmap(raw, width, height);

            // Otherwise build a float intensity array and normalise to 8-bit grey.
        ...
        private Bitmap BuildRgbaBitmap(byte[] raw, int width, int height)
        {
            int n = width * height;
            Bitmap bmp = new Bitmap(width, height, PixelFormat.Format32bppArgb);
            BitmapData bd = bmp.LockBits(new Rectangle(0, 0, width, height),
                                         ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
            byte[] px = new byte[n * 4];
            // DM stores B,G,R,A. GDI+ 32bppArgb in memory is also B,G,R,A (little-endian).
            for (int i = 0; i < n; i++)
            {
                int s = i * 4, d = i * 4;
                byte b = raw[s], g = raw[s + 1], r = raw[s + 2]; // A in raw[s+3] often 0
                px[d] = b; px[d + 1] = g; px[d + 2] = r; px[d + 3] = 255;
            }
            Marshal.Copy(px, 0, bd.Scan0, px.Length);
            bmp.UnlockBits(bd);
            return bmp;
